...
export default function MoviesList({ initialData, initialPage = 1, totalPages: totalPagesProp = 1 }) {
  ...
  const rawMovies = (data?.movies || []).map((m) => ({ ...m, mediaType: 'movie' }));
  ...
  const filteredMovies = useMemo(() => {
    if (!searchQuery.trim()) return rawMovies;
    const q = searchQuery.toLowerCase().trim();
    return rawMovies.filter((m) => {
      const title = (m.title || '').toLowerCase();
      const originalTitle = (m.originalTitle || '').toLowerCase();
      return title.includes(q) || originalTitle.includes(q);
    });
  }, [rawMovies, searchQuery]);
  ...
  return (
    <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 pt-20 sm:pt-24 lg:pt-28 pb-12 text-left flex flex-col gap-8 min-h-screen bg-transparent">
      ...
      <section className="relative overflow-hidden rounded-3xl border border-white/[0.08] bg-gradient-to-br from-brand-primary/15 via-luxury-900/60 to-luxury-950 p-6 sm:p-10 shadow-2xl backdrop-blur-xl">
        ...
        <div className="relative z-10 max-w-3xl flex flex-col items-start gap-4">
          ...
          <p className="text-xs sm:text-sm text-slate-300 leading-relaxed max-w-2xl font-light">
            Browse the complete collection of high-definition Korean movies, box-office blockbusters, and award-winning cinema with verified Sinhala (<span className="text-brand-primary-light font-semibold">සිංහල උපසිරැසි</span>) and English SRT subtitle downloads.
          </p>
          ...
          <div className="flex flex-wrap items-center gap-2 pt-1">
            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-xl bg-white/[0.04] border border-white/[0.08] text-[10px] font-bold text-slate-300 backdrop-blur-sm">
              <Sparkles className="w-3 h-3 text-brand-accent" /> Free SRT Downloads
            </span>
            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-xl bg-white/[0.04] border border-white/[0.08] text-[10px] font-bold text-slate-300 backdrop-blur-sm">
              <Star className="w-3 h-3 text-amber-400 fill-amber-400" /> IMDb Verified Ratings
            </span>
            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-xl bg-white/[0.04] border border-white/[0.08] text-[10px] font-bold text-slate-300 backdrop-blur-sm">
              <Film className="w-3 h-3 text-brand-secondary" /> Synchronized Timing
            </span>
          </div>
        </div>
      </section>
        ...
        {isLoading ? (
          ...
          >
            ...
            <p className="text-xs text-slate-400 max-w-sm leading-relaxed mb-4">
              We couldn&apos;t find any movies matching your current filter or search criteria.
            </p>
            {(searchQuery || country) && (
              <button
      ...
      {totalPages > 1 && (
        <nav
          aria-label="Movie listing pagination"
